...
contract praxisFACTORY is FlashLoanSimpleReceiverBase, Ownable {
    ...
    address immutable ERC20_TO_DEPOSIT_POLYGON; //0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619; // WETH
    address immutable ERC20_TO_BORROW_POLYGON; // 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;  // USDC
    address immutable AAVE_POOL_POLYGON; // 0x794a61358D6845594F94dc1DB02A252b5b4814aD;
        ...
        returns (bool)
    {
        ...
        IERC20(ERC20_TO_DEPOSIT_POLYGON).approve(
            AAVE_POOL_POLYGON, 3 * amount_deposited
        );
        supply_WETH_Pool(address(this), 3 * amount_deposited);
        uint256 amount_borrow = 2 * (amount_deposited);
        borrow_USDC_Pool(address(this), amount_borrow);
        (uint256 amountIn_USDC) = swapExactOutputSingle(
            2 * (amount_deposited),
            amount_borrow,
    ...
    function get_CONTRACT_WETH_POSITION() internal returns (uint256) {
        (uint256 a,,,,,) =
            IPool(AAVE_POOL_POLYGON).getUserAccountData(address(this));
        return a;
    }

    function get_CONTRACT_USDC_POSITION() internal returns (uint256) {
        (, uint256 a,,,,) =
            IPool(AAVE_POOL_POLYGON).getUserAccountData(address(this));
            return a;
    }

    function CALCULATE_TOKENS_TO_MINT(uint256 amountInEth, uint256 ethPrice)
        internal
        returns (uint256)
    {
    ...
    function mintETHUP3X(address _User) internal {
        ...
        uint256 ETHUP3X_TOKENS = CALCULATE_TOKENS_TO_MINT(amountInEth, ethPrice);
        praxisERC20(PRAXIS_ERC20).mint(_User, ETHUP3X_TOKENS);

        count_Positions[_User]++;
    }

    function deployERC20() external onlyOwner returns (address _praxisERC20) {
        // _praxisERC20 = new praxisERC20();
        bytes memory bytecode = type(praxisERC20).creationCode;
        bytes32 salt = keccak256(
            abi.encodePacked(ERC20_TO_DEPOSIT_POLYGON, ERC20_TO_BORROW_POLYGON)
        );
        assembly {
            _praxisERC20 := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
    }
